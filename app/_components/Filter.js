...
function Filter() {
    ...
    const router = useRouter(); //*to navigate for special router
    const pathname = usePathname(); //cabins
    const activeFilter = searchParams.get("capacity") ?? "all";
    function handleFilter(filter) {
        const params = new URLSearchParams(searchParams);
        params.set("capacity", filter);
        router.replace(`${pathname}?${params.toString()}`, { scroll: false }); // ("cabins?capacity=filter"), to don't scroll up
    }
    return (
...
function Button({ filter, handleFilter, activeFilter, children }) {
    return <button className={`px-5 py-2 hover:bg-primary-200 ${filter === activeFilter ? "bg-primary-700 text-primary-50" : ""}`} onClick={() => handleFilter(`${filter}`)}>
        {children}</button>
}
